...
router.get('/', verifyToken, async (req, res) => {
    try {
        ...
        const [summary] = await req.db.query(countSql, countParams);
        const [total] = await req.db.query(totalSql, countParams);
...
router.get('/:id', verifyToken, async (req, res) => {
    try {
        const [spkRows] = await req.db.query(`
            SELECT s.*, u1.name AS assigned_name, u2.name AS created_by_name
            FROM spk s
        ...
        const [handover] = await req.db.query(
            'SELECT * FROM spk_handovers WHERE spk_id = ? ORDER BY created_at DESC LIMIT 1',
            [req.params.id]
        );

        res.json({ ...spkRows[0], logs, payments, handover: handover[0] || null });
    } catch (error) {
...
router.post('/', verifyToken, requireRole(['admin', 'kasir', 'operator']), async (req, res) => {
    ...
    try {
        ...
        const total_biaya = (biaya_cetak || 0) + (biaya_material || 0) + (biaya_finishing || 0) + (biaya_desain || 0) + (biaya_lainnya || 0);
        ...
        `, [id, spk_number, customer_id || null, customer_name, customer_phone || null, customer_company || null, product_name, product_qty, product_unit, kategori, specs_material || null, specs_finishing || null, specs_notes || null, biaya_cetak, biaya_material, biaya_finishing, biaya_desain, biaya_lainnya, total_biaya, dp_amount, sisa_tagihan, priority, assigned_to || null, req.user.id, deadline || null]);

        await conn.query(
            'INSERT INTO spk_logs (spk_id, user_id, action, description, new_value) VALUES (?, ?, ?, ?, ?)',
            [id, req.user.id, 'STATUS_CHANGE', 'SPK Baru Dibuat', 'Menunggu Antrian']
        );

        if (dp_amount > 0) {
            await conn.query(
                'INSERT INTO spk_payments (spk_id, payment_type, method, amount, paid_by) VALUES (?, ?, ?, ?, ?)',
                [id, 'DP', 'Tunai', dp_amount, req.user.id]
            );
        }
...
router.patch('/:id/finalize', verifyToken, requireRole(['admin', 'kasir']), async (req, res) => {
    try {
        ...
        const [current] = await req.db.query('SELECT dp_amount FROM spk WHERE id = ?', [req.params.id]);
        if (!current.length) return res.status(404).json({ message: 'SPK tidak ditemukan' });
        ...
        `, [biaya_cetak || 0, biaya_material || 0, biaya_finishing || 0, biaya_desain || 0, biaya_lainnya || 0, total_biaya, sisa_tagihan, req.params.id]);
...
router.post('/:id/pay', verifyToken, requireRole(['admin', 'kasir']), async (req, res) => {
    ...
    try {
        ...
        await conn.query('UPDATE spk SET sisa_tagihan = ?, dp_amount = dp_amount + ? WHERE id = ?', [newSisa, amount, req.params.id]);
        ...
        await conn.query(
            ...
            [cashFlowId, date, payment_type === 'DP' ? 'DP SPK' : 'Pelunasan SPK', amount, `${payment_type || 'Pelunasan'} SPK ${current[0].spk_number}`, req.params.id]
        );
...
router.post('/:id/handover', verifyToken, requireRole(['admin', 'kasir']), async (req, res) => {
    ...
    try {
        ...
        await conn.query(`
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        `, [req.params.id, received_by_name, received_by_phone || null, signature_data || null, photo_evidence || null, notes || null, req.user.id]);
...
router.get('/payments/qris', verifyToken, async (req, res) => {
    try {
        const [transactions] = await req.db.query(`
            SELECT p.*, s.spk_number, s.customer_name, u.name AS cashier_name
            FROM spk_payments p
        ...
        const [stats] = await req.db.query(`
            SELECT COUNT(*) as total_tx, SUM(CASE WHEN status='Berhasil' THEN 1 ELSE 0 END) as success_tx, SUM(CASE WHEN status='Berhasil' THEN amount ELSE 0 END) as total_amount
