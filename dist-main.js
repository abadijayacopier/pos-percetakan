...
var serverProcess;
var setupWindow;
var isSetupMode = process.argv.includes("--setup");
...
ipcMain.handle("check-mariadb", async () => {
  return new Promise((resolve) => {
    ...
    function tryNext() {
      ...
      exec(`"${loc}" --version 2>&1`, { timeout: 3e3 }, (err, stdout) => {
    ...
    exec("sc query mysql", { timeout: 3e3 }, (err, stdout) => {
...
ipcMain.handle("download-mariadb", async (event) => {
  ...
  function sendLog(msg) {
    win.send("setup-log", msg);
  }
  function sendProgress(data) {
    win.send("download-progress", data);
  }
  return new Promise((resolve) => {
    ...
    const request = https.get(url, (res) => {
      ...
      file.on("finish", () => {
        ...
        const msi = spawn("msiexec", [
          ...
          "PORT=3306",
          "/L*V",
          path.join(os.tmpdir(), "mariadb-install.log")
...
async function importDatabase(log) {
  return new Promise((resolve) => {
    ...
    for (const loc of mysqlLocations) {
      try {
        require("child_process").execSync(`"${loc}" --version`, { timeout: 3e3 });
        mysqlBin = loc;
        break;
      } catch (_) {
      }
...
function startBackend() {
  ...
  const serverPath = isPackaged ? path.join(process.resourcesPath, "app", "server", "index.js") : path.join(__dirname, "server", "index.js");
  const dbDir = isPackaged ? path.join(app.getPath("userData"), "database") : path.join(__dirname, "server", "database");
